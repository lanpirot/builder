classdef GSubTree
    ...
    methods
        ...
        function obj = GSubTree(sub, subinfos)
        ...
        function obj = recursive_subtree(obj, subinfos)
            ...
            for i = 1:length(tmpchildren)
                if subinfos.isKey({tmpchildren(i)})
                    obj.children{end + 1} = GSubTree(tmpchildren(i), subinfos).recursive_subtree(subinfos);
                end
            end
        end
        ...
        function [obj, model_handle, additional_level] = build(obj, slx_id)
            global name2subinfo_complete
            model_name = slx_id.sub_name;
            ...
            [slx_id, additional_level] = GModelMutator.copy_to_root(slx_id.sub_name, slx_id.model_path, obj.identity, slx_id);
            GModelMutator.make_subsystem_editable(slx_id.get_qualified_name());
            set_param(model_name, 'Lock', 'off')
            set_param(model_name, "LockLinksToLibrary", "off")
            GModelMutator.annotate(slx_id.get_qualified_name(), "Copied system from: " + obj.identity.hash() + newline + "to: " + slx_id.hash())
        ...
        function obj = build_sub(obj, copy_to, slx_id, slx_parents, dry_build)
            ...
            if dry_build
                ...
                copy_into = GModelMutator.copy_to_non_root(copy_into, copy_from, copied_element, mapping);
                obj.synthed_identity = copy_into;
                GModelMutator.make_subsystem_editable(copy_into.get_qualified_name());
                GModelMutator.annotate(copy_into.get_qualified_name(), "Copied system from: " + copy_from.hash() + newline + "to: " + copy_to.hash())
            end
